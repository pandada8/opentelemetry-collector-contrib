...
func (e *baseExporter) start(ctx context.Context, host component.Host) error {
	...
	client.Transport = &redirectLoggingTransport{next: client.Transport, logger: e.logger}
	e.client = client
	return nil
}
...
func (e *baseExporter) export(ctx context.Context, requestURL string, request []byte, partialSuccessHandler partialSuccessHandler) error {
	// Read the headers actually written by the transport. Response.Request can
	// describe a different internal request after a transport retry.
	...
	ctx = httptrace.WithClientTrace(ctx, &httptrace.ClientTrace{
		WroteHeaderField: func(key string, values []string) {
			...
			case strings.EqualFold(key, "Content-Length"):
				if size, err := strconv.ParseInt(values[0], 10, 64); err == nil {
					headerMu.Lock()
					bodyLengths = append(bodyLengths, size)
					headerMu.Unlock()
				}
	...
	if resp.StatusCode == http.StatusRequestEntityTooLarge {
		// Preserve every emitted Content-Length so retries are visible. A size
		// of -1 means the transport did not report a Content-Length.
		bodySize := int64(-1)
		headerMu.Lock()
		writtenLengths := append([]int64(nil), bodyLengths...)
		writtenEncoding := contentEncoding
		headerMu.Unlock()
		if len(writtenLengths) > 0 {
			bodySize = writtenLengths[len(writtenLengths)-1]
		}
		responseRequestSize := int64(-1)
		if resp.Request != nil {
			responseRequestSize = resp.Request.ContentLength
		}
		e.logger.Error("OTLP HTTP request rejected with 413",
			zap.Int("http_status_code", resp.StatusCode),
			zap.Int("request_body_uncompressed_bytes", len(request)),
			zap.Int64("request_body_bytes", bodySize),
			zap.Int64s("request_body_bytes_per_attempt", writtenLengths),
			zap.Int64("response_request_content_length", responseRequestSize),
			zap.String("request_content_encoding", writtenEncoding),
